...
interface ProgramAlumniTableProps {
  data: ProgramAlumni[];
  isLoading: boolean;
  onEdit: (ProgramAlumni: ProgramAlumni) => void;
  onDelete: (ProgramAlumni: ProgramAlumni) => void;
}

export function ProgramAlumniTable({ data, isLoading, onEdit, onDelete }: ProgramAlumniTableProps) {
  const columnHelper = createColumnHelper<ProgramAlumni>();
  ...
  const columns = [
    ...
    columnHelper.accessor('name', {
      header: 'Nama Program Alumni',
      cell: (info) => {
        const major = info.getValue();
        return major && major.trim() ? (
          <div className="font-medium">{major}</div>
        ) : (
          <span className="text-muted-foreground italic">(ProgramAlumni belum diberi nama)</span>
        );
      },
    }),
