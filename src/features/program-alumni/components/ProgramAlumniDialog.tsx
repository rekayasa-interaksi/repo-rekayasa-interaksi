...
import { useCreateProgramAlumni, useUpdateProgramAlumni } from '../queries';
import { ProgramAlumniFormSchema } from '../schemas';
import type { ProgramAlumni, ProgramAlumniFormData } from '../types';

interface ProgramAlumniDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  mode: 'create' | 'edit';
  defaultValues?: ProgramAlumni;
}

export function ProgramAlumniDialog({
  open,
  onOpenChange,
  mode,
  defaultValues,
}: ProgramAlumniDialogProps) {
  const createMutation = useCreateProgramAlumni();
  const updateMutation = useUpdateProgramAlumni();

  const isSubmitting = createMutation.isPending || updateMutation.isPending;
  const isEditing = mode === 'edit';

  const form = useForm<ProgramAlumniFormData>({
    resolver: zodResolver(ProgramAlumniFormSchema),
  ...
  const onSubmit = async (data: ProgramAlumniFormData) => {
    try {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 p-6">
            <div className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-base font-medium">Nama ProgramAlumni</FormLabel>
                    <FormControl>
                      <div className="group relative">
                        <Input 
                          placeholder="Masukkan nama programAlumni" 
                          {...field} 
