...
import { useDeleteMajorCampus } from '../queries';
...
export function DeleteConfirmationDialog({
  ...
  const deleteMutation = useDeleteMajorCampus();
  const isDeleting = deleteMutation.isPending;

  const handleDelete = async () => {
    await deleteMutation.mutateAsync(majorCampus.id);
    onOpenChange(false);
  };
      ...
      <DialogContent className="max-w-md p-0 overflow-hidden">
        <div className="bg-red-500/10 px-6 pt-6 pb-4 flex flex-col items-center">
          ...
          <DialogHeader className="text-center">
            <DialogTitle className="text-xl font-bold">Hapus Jurusan</DialogTitle>
          </DialogHeader>
        </div>
        
        <div className="px-6 py-4">
          <DialogDescription className="text-center text-base">
            Apakah Anda yakin ingin menghapus jurusan <span className="font-semibold text-foreground">"{majorCampus.major}"</span>?
